...
if($sessionAdmin!="")
{
	...
	$hid_companies = (isset($HTTP_GET_VARS['hid_companies'])?quote_smart($HTTP_GET_VARS['hid_companies']):"");
	...
	$qry_select_users ="select id,username,password from cs_customerserviceusers where 1 order by id";
...
function validation() {
	...
	else if (document.adduser.username.value!=""&&(!func_vali_pass(document.adduser.username)))
		{
		
		alert("Special characters not allowed for username");
		document.adduser.username.focus();
		document.adduser.username.select();
		return false;
		}
	...
	else if (document.adduser.password.value!=""&&(!func_vali_pass(document.adduser.password)))
		{
		 ...
		 <tr>
		  ...
		  <tr>
			...
			<select name="companyname" style="font-family:verdana;font-size:10px;" multiple>
			  ...
			  <?php	
				$qrt_select_company = "select userid,companyname from cs_companydetails where 1 order by companyname";
		  ...
		  <tr><td align="center" height="40" valign="bottom">&nbsp;&nbsp;&nbsp;<input type="image" id="adduser" SRC="<?=$tmpl_dir?>/images/submit.jpg"></input>
		   </td>
		</tr>
		</table>
	</form>
