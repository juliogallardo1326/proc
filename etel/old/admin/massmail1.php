...
$company_table_sql = "select distinct email,companyname,username,password,send_mail,ReferenceNumber, userId from cs_companydetails where 0";
if($_REQUEST['cd_view']=='A')
{
	$company_table_sql = "select distinct email,companyname,username,password,send_mail,ReferenceNumber, userId from cs_companydetails where cd_ignore = 0";
...
if($_REQUEST['companyname'][0]=='AL')
{
	...
	$company_table_sql = "select distinct email,companyname,username,password,send_mail,ReferenceNumber, userId from ". $sql_info['sql_from']." Where ".$sql_info['sql_where'];
...
else if($userIdList)
{
	...
	$company_table_sql = "select distinct email,companyname,username,password,send_mail,ReferenceNumber, userId from cs_companydetails where userid in ($userIdList)";
}
