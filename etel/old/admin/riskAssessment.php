...
function genReportSection($rp_source_query)
{
	...
	foreach($source_data as $key=>$data)
	{
		...
		if($Approved_Transactions>0) $section[$key]['per']['val'] = round(floatval($data_parts[0])/$Approved_Transactions,4)*100;
		if($Approved_Amount>0) $section[$key]['apr']['val'] = round(floatval($data_parts[1])/$Approved_Amount,4)*100;
		if($Total_Transactions>0) $section[$key]['tper']['val'] = round(floatval($data_parts[0])/$Total_Transactions,4)*100;
		if($Total_Amount>0) $section[$key]['tapr']['val'] = round(floatval($data_parts[1])/$Total_Amount,4)*100;
...
function getHalfProcessingTime($compId,$period)
{
	...
	$comp_sql = "";
	if(intval($compId) >-1) $comp_sql = " AND t.`userId` = '$compId'";
