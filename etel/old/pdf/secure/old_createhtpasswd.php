...
$sql="SELECT * FROM `etel_dbsmain`.`cs_company_sites` as s WHERE `cs_gatewayId` = ".$_SESSION["gw_id"]." AND `cs_reference_id` = '$mt_reference_id' ";
...
$sql="SELECT * FROM $trans_table WHERE $affiliation AND `td_recur_processed` = 0 AND `td_recur_next_date`>=CURDATE()";
$result=mysql_query($sql,$cnn_cs) or die(mysql_errno().": ".mysql_error()."<br>Cannot execute query");
$num = mysql_num_rows($result);
//if($num<1) die("UNF");
...
$result=mysql_query("UPDATE `etel_dbsmain`.`cs_company_sites` set `cs_user_checksum`= '$hash' WHERE `cs_gatewayId` = ".$_SESSION["gw_id"]." AND `cs_reference_id` = '$mt_reference_id'",$cnn_cs) or die(mysql_errno().": ".mysql_error()."<br>Cannot execute query");
//$ht->addLogin();
die("NEW");
?>