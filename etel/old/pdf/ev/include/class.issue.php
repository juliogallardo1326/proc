...
class Issue
{
     ...
     * @return  boolean
     */
    function exists($issue_id, $check_project = true, $by_field='iss_id')
    {
		$where = "$by_field =" . Misc::escapeInteger($issue_id);
		if($by_field=='iss_summary')
			$where = "$by_field like '" . Misc::escapeString($issue_id) . "'";
    ...
    function setProject($issue_id, $prj_id)
    {
        $issue_id = Misc::escapeInteger($issue_id);
    ...
    function getReplyDetails($issue_id)
    {
        ...
        $stmt = "SELECT
                    iss_created_date,
                    CONCAT(en_firstname,' ', en_lastname) AS reporter,
    ...
    function updateControlStatus($issue_id)
    {			
		$stmt = "SELECT
			iss_usr_id, sup_usr_id, iss_prj_id
		 FROM
			(
		 ...
		 WHERE
			iss_id=" . Misc::escapeInteger($issue_id) . "
			and sup_iss_id = iss_id
		 ORDER BY
			sup_id DESC
		 Limit 1
			";
		
		list($reporter_id, $last_reply_id, $prj_id) = $GLOBALS["db_api"]->dbh->getRow($stmt);

		$iss_control_status = 'Unanswered';
		if(User::getRoleByUser($reporter_id,$prj_id)<User::getRoleByUser($last_reply_id,$prj_id) && User::getRoleByUser($last_reply_id,$prj_id)>=User::getRoleID('Developer'))
			$iss_control_status = 'Answered';
			
        $stmt = "UPDATE
                    " . APP_DEFAULT_DB . "." . APP_TABLE_PREFIX . "issue
                 SET
					iss_control_status='$iss_control_status'
                    ";
        $stmt .= "WHERE
                    iss_id=" . Misc::escapeInteger($issue_id);
		$res = $GLOBALS["db_api"]->dbh->query($stmt);
                ...
                switch ($options[$field_name]['filter_type']) {
                    ...
                    case 'not_in_past':
						...
						(
							...
							(
								(
									(UNIX_TIMESTAMP('" . Date_API::getCurrentDateGMT() . "') - UNIX_TIMESTAMP(iss_$field_name)) > (" .
                            		Misc::escapeInteger($options[$field_name]['time_period']) . "*3600)
								)
							)
						)";
    ...
    function getReportersByIssues(&$result)
    {
        ...
        $stmt = "SELECT
                    iss_id,
                    CONCAT(en_firstname,' ', en_lastname, ' <', en_email, '>') AS usr_full_name
                 FROM
                    " . APP_DEFAULT_DB . "." . APP_TABLE_PREFIX . "issue,
                    " . ETEL_USER_TABLE_NOSUB . "
                 WHERE
    ...
    function canAccess($issue_id, $usr_id)
    {
        ...
        } elseif ((Auth::getCurrentRole() <= User::getRoleID("Standard User")) && (Project::getSegregateReporters($prj_id)) &&
                ($details['iss_usr_id'] != $usr_id) && (!Issue::isAssignedToUser($issue_id, $usr_id)) && (!Authorized_Replier::isUserAuthorizedReplier($issue_id, $usr_id))) {
            return false;
        } else {
            $return = true;
        }
    ...
    function getIssueBy($string,$by_field)
    {	
		if(!$by_field) return false;
		$where = "$by_field ='" . Misc::escapeString($string). "'";
