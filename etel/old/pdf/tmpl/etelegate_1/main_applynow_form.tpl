...
<form name="Frmcompany" id="Frmcompany" onsubmit="return submitform(this)" action="{if $app.submit}{$app.submit}{else}add_merchant.php?submit=1{/if}" method="post">
  <table style="border: 1px solid rgb(80, 134, 15); background-color: white;" align="center" border="0" cellpadding="0" width="410">
    <tbody>
      <tr>
		...
		{if $app.title}{$app.title}{else}Application Form{/if}
		</font></div></td>
      </tr>
      <tr>
	  ...
	  {foreach item=field key=title from=$app.fields}     
	  <tr>
        <td class="bodytext" align="right" height="30" valign="center" width="404">{$title}</td>
        <td align="left" height="30" width="252"><input title="{$title}" name="{$field.name}" id="{$field.name}" style="font-size: 10px; font-family: arial;" value="{$field.value}" maxlength="100" valid="req"><i class="bodytext">{$field.detail}</i></td>
      </tr>
	  {/foreach}
      <tr>
