...
$my_sql['joins'] = array(
		...
		array("table"=>"cs_company_banks as cb",
				"on"=>
					array(
						array("field_a"=>"cb.userId","field_b"=>"cd.userId","compare"=>"="),
...
$my_sql['manip']["res_disc"] = array("source" => "cb_config","function"=>"smart_array_path","params"=>array('default','Processor','disct'));
$my_sql['manip']["res_trans"] = array("source" => "cb_config","function"=>"smart_array_path","params"=>array('default','Processor','trans'));
...
$my_sql['return']["01|Ref ID"] = array("source" => "cd.ReferenceNumber","column"=>"ReferenceNumber");
$my_sql['return']["02|Company Name"] = array("source" => "cd.companyname","column"=>"companyname","crop"=>25);

$my_sql['return']["03|Sites"] = array("source" => "(select count(cs_ID) as sites from etel_dbsmain.cs_company_sites AS cs where cs_company_id = cd.userId group by cs_company_id) as sites","column"=>"sites");


$my_sql['return']["04|Docs"] = array("source" => "(select count(distinct file_type) as docs from cs_uploaded_documents AS ud where ud.user_Id = cd.userId group by ud.user_Id) as docs","column"=>"docs");
...
if($export_data)
{
	if(in_array($export_data,array('full','company','company2')))
	{
		...
		{
			$my_sql['return']["02|SiteInfo"] = array("source" => "(select group_concat(distinct `cs_name` SEPARATOR ', ') as site_info from etel_dbsmain.cs_company_sites AS cs where cs_company_id = cd.userId group by cs_company_id) as site_info","column"=>"site_info");
...
$my_sql['where']['ea.ea_affiliate_ID'] = array("value" => $companyInfo['en_ID'], "compare" => "=");
