...
// changePassword.php:	The page functions for the company account setup. 
...
$usernameO = $companyInfo['en_username'];
$passwordO = $companyInfo['en_password'];


$password = $HTTP_GET_VARS['password'];
$Npassword = $usernameO.quote_smart($HTTP_GET_VARS['Npassword']);
if($password == $passwordO)
{
	$show_sql =mysql_query("update cs_entities set reseller_password=md5('$Npassword') where en_ID=$resellerLogin");
	$_SESSION["gw_user_password"] = $Npassword;
	$msgtodisplay="Password has been changed for reseller ";			
	$outhtml="y";
...
{
	$invalidlogin="<br><font face='verdana' size='1' color='red'>Please enter correct password in the old password field !! </font>";
}  
...
function validation(){
  trimSpace(document.Frmlogin.password);
...
if (document.Frmlogin.Npassword.value!=""&&(!func_vali_pass(document.Frmlogin.Npassword)))
  ...
  trimSpace(document.Frmlogin.Npassword1);
...
<br>	<form action="changePassword.php" method="GET" onsubmit="return validation()" name="Frmlogin" >
