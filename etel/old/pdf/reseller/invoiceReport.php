...
function funcPDF(sArgument) {
	objForm		=	document.update_shipping;
	objForm.method 	=	"post";
	objForm.action 	=	"printpdf.php";
	objForm.target	=	"_blank";
	objForm.hdArg.value	=	sArgument;	
	objForm.submit();
}
</script>
<?php
...
$selected_company = "";
if($companyid) {
	$qry_select_details ="Select a.transactionDate,a.status,a.cancelstatus,a.amount,b.chargeback,b.transactionfee,b.voiceauthfee,b.companyname,b.credit,b.discountrate,b.reserve from cs_transactiondetails as a,cs_companydetails as b where a.userid=b.userid and a.userid=$companyid ";// and a.transactionDate >='$str_from_date' and a.transactionDate <='$str_to_date'" ;
...
<input type="hidden" name="hdArg" value="">
...
<?php 	$iloop = 0;
		...
		//------------------  Values using for PDF creation ----
		//------------------------------------------------------
		
		$iTotalTransactions		=	0;
		$iApprovedTransactions	=	0;
		$iDeclinedTransactions	=	0;
		...
		{	
			...
			$iloop = $iloop +1;
			$iTotalTransactions		=	$iTotalTransactions	+ 1;	
			...
			if (($str_temp_date != $str_date || $str_temp_company != $str_company) && $str_temp_date != "" ) {
				...
				$sArgument = $companyid.",";
				$sArgument .= ($i_num_days_back != "" && $str_frequency == "W") ?  ($str_from_date ." - ". $str_to_date) : $str_temp_date;
				$sArgument .= ",";
				$sArgument .= $iTotalTransactions.",";
				$sArgument .= $iApprovedTransactions.",";
				$sArgument .= $iDeclinedTransactions.",";
				$sArgument .= $i_trans_fee.",";
				$sArgument .= $i_discount_rate.",";
				$sArgument .= $i_charge_back_fee.",";
				$sArgument .= $i_credit.",";
				$sArgument .= $i_misc_fee.",";
				$sArgument .= $i_reserve.",";
				$sArgument .= $i_total_amt.",";
				$sArgument .= $i_total_deduction.",";
				$sArgument .= $i_total_amt - $i_total_deduction.",";
			...
			?>		
				<tr>
				<td class="leftbottomright" height="30"><font size="1" face="Verdana" ><a href="javascript:funcPDF('<?= $sArgument ?>')"><?= ($i_num_days_back != "" && $str_frequency == "W") ?  ($str_from_date ." - ". $str_to_date) : $str_temp_date?></a></font></td>
		...
		<?php
			...
			if ($str_status == "A") {
				...
				$iApprovedTransactions	=	$iApprovedTransactions + 1;
			}
			if ($str_status == "D") {
				$iDeclinedTransactions	=	$iDeclinedTransactions	+	1;
			}
		...
		?>
			<tr>
			<td class="leftbottomright" height="30"><font size="1" face="Verdana" ><a href="javascript:funcPDF('<?= $sArgument ?>')"><?= ($i_num_days_back != "" && $str_frequency == "W") ?  ($str_from_date ." - ". $str_to_date) : $str_date?></a></font></td>
			<td class="rightbottomtd" align="right"><font size="1" face="Verdana" ><?=formatMoney($i_total_amt);?></font></td>
			<td class="rightbottomtd" align="right"><font size="1" face="Verdana" ><?=formatMoney($i_trans_fee);?></font></td>
			<td class="rightbottomtd" align="right"><font size="1" face="Verdana" ><?=formatMoney($i_voice_auth_fee);?></font></td>
			<td class="rightbottomtd" align="right"><font size="1" face="Verdana" ><?=formatMoney($i_charge_back_fee);?></font></td>
