...
function execute_refund($transInfo,$reason)
{
	...
	$sql="SELECT * FROM `cs_companydetails` as c left join `etel_dbsmain`.`cs_company_sites` as s on s.cs_company_id = c.`userId` WHERE `cs_gatewayId` = ".$_SESSION["gw_id"]." AND c.`userId` = '".$transInfo['userId']."' and s.`cs_ID` = '".$transInfo['td_site_ID']."'";
