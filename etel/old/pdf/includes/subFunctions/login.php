...
function general_login($username,$password,$usertype,$gatewayid,$reset_session=true,$redirect=true)
{
	...
	$sql = "SELECT * FROM `etel_gateways` where `gw_id`= '$gatewayid'";
	$result=sql_query_read($sql,$cnn_cs) or dieLog(mysql_error(). " $sql");
	$gw=mysql_fetch_assoc($result);
	mysql_select_db($gw["gw_database"],$cnn_cs) or dieLog("Unable to connect database ".$gw["gw_database"]); 
	...
	if($userInfo = mysql_fetch_assoc($result))
	{
		...
		switch ($userInfo['en_type']) 
		{
			case "merchant":
				...
				else if($show_val['cd_completion']<=9) header("location:".$etel_domain_path."/ledger.php");
				else header("location:".$etel_domain_path."/ledger.php");
				exit();
				break;
