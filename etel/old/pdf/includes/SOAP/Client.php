                         ...
                         $soapAction = false)
    {
        ...
        if ($this->__endpointType == 'wsdl') {
            ...
            if ($this->__options['input'] == 'parse') 
			{
				...
				{
					//lowercase all the parameter keys
					$temp = array();
                    ...
                    foreach ($opData['input']['parts'] as $name => $part) 
					{
                        ...
                        if (isset($params[strtolower($name)]) ||
                            $this->_wsdl->getDataHandler($name, $part['namespace'])) 
						{
                            $nparams[$name] =& $params[strtolower($name)];
                        } 
						
/*  If a parameter is missing we don't care: stupid SOAP it's none of your business
						else 
						{
