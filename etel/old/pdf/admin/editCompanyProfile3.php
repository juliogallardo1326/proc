...
if(1)//$adminInfo['li_level'] == 'full' || $resellerInfo['isMasterMerchant'])
{
	if ($str_update == "yes") {
		...
		$cd_next_pay_day=(isset($HTTP_POST_VARS['cd_next_pay_day'])?quote_smart($HTTP_POST_VARS['cd_next_pay_day']):"");
		...
		$cd_custom_orderpage=(isset($HTTP_POST_VARS['cd_custom_orderpage'])? html_entity_decode(quote_smart($HTTP_POST_VARS['cd_custom_orderpage'])):"");
		$gateway_id=(isset($HTTP_POST_VARS['gateway_id'])?quote_smart($HTTP_POST_VARS['gateway_id']):"");
		$cd_completion=(isset($HTTP_POST_VARS['cd_completion'])?quote_smart($HTTP_POST_VARS['cd_completion']):"");
		$cd_allow_gatewayVT=(isset($HTTP_POST_VARS['cd_allow_gatewayVT'])?quote_smart($HTTP_POST_VARS['cd_allow_gatewayVT']):"");
		
		if(!$gateway_id) $gateway_id = $_SESSION['gw_id'];
		
		if($trans_activity && !$bank_check) $bank_check=18;
...
{
	$sql = "select * from cs_bank where bank_id='$bank_Creditcard' $bank_sql_limit";
	$result=mysql_query($sql) or dieLog(mysql_error());
	...
	else if($cc_BankInfo['bk_defaults']) dieLog("Invalid Bank Defaults: ".$cc_BankInfo['bk_defaults'],"Invalid Bank Defaults");
}
...
// End Email
		...
		if($adminInfo['li_level'] != 'full')
		{
			$bank_Creditcard = $companyInfo['bank_Creditcard'];
			$bank_check = $companyInfo['bank_check'];	
			$ch_merchant_trans_fees = $companyInfo['ch_merchant_trans_fees'];
			$ch_reseller_trans_fees = $companyInfo['ch_reseller_trans_fees'];
			$ch_total_trans_fees = $companyInfo['ch_total_trans_fees'];
		...
		$qry_update_user  =  "update cs_companydetails left join {$database["database_main"]}.cs_company_sites on cs_company_id = userId set $completion cc_underchargeback='$cc_underchargeback', cc_overchargeback='$cc_overchargeback', cc_chargeback='$cc_chargeback', cc_discountrate='$cc_discountrate', cc_reserve='$cc_reserve',	ch_chargeback='$ch_chargeback', ch_discountrate='$ch_discountrate', ch_reserve='$ch_reserve', web_chargeback='$web_chargeback', web_discountrate='$web_discountrate', web_reserve='$web_reserve', ";
		...
		$qry_update_user .=  "cd_allow_gatewayVT='$cd_allow_gatewayVT',`gateway_id`='$gateway_id', bank_check = '$bank_check', cd_web900bank='$cd_web900bank',suspenduser='$suspend',cs_monthly_charge = '$cs_monthly_charge', we_billingdescriptor='$we_billingdescriptor', cc_visa_billingdescriptor='$cc_visa_billingdescriptor', cc_master_billingdescriptor='$cc_master_billingdescriptor',ch_billingdescriptor='$ch_billingdescriptor',processing_currency='$processingcurrency',send_mail='$strUnsubscribe',send_ecommercemail = '$send_ecommercemail',bank_shopId = '$strBankShopId',bank_Username='$strBankUsername',bank_Password='$strBankPassword',bank_Creditcard='$bank_Creditcard', cancel_ecommerce_letter = '$cancelecommerce_checked', cd_enable_price_points ='$cd_enable_price_points', cd_allow_rand_pricing='$cd_allow_rand_pricing', cd_custom_recur = '$cd_custom_recur', atm_verify = '$atm_verify', integrate_check='$check_integrate',block_virtualterminal='$virtualterminal_off',block_recurtransaction='$recurtransactionoff',block_rebilltransaction='$rebilltransactionoff',";
		$qry_update_user .=  "cd_next_pay_day='$cd_next_pay_day', cd_fraudscore_limit='$cd_fraudscore_limit', cd_orderpage_settings='$cd_orderpage_settings', cd_orderpage_useraccount='$cd_orderpage_useraccount', cd_approve_timelimit='$cd_approve_timelimit', cd_custom_orderpage=$cd_custom_orderpage, cd_custom_contract='$cd_custom_contract', cd_pay_status='$cd_pay_status',cd_ignore='$cd_ignore', cd_max_transaction='$cd_max_transaction',cd_max_volume='$cd_max_volume',contact_email='$contact_email',customer_service_phone='$customer_service_phone',cd_payperiod='$cd_payperiod',cd_paystartday='$cd_paystartday',cd_paydelay='$cd_paydelay',cd_paydaystartday='$cd_paydaystartday',cd_rollover='$cd_rollover',cd_wirefee='$cd_wirefee',cd_appfee='$cd_appfee',cd_completion='$cd_completion'";

		if($adminInfo['li_level'] == 'full') $qry_update_user .= ",activeuser='$trans_activity'";

		$qry_update_user .=  " where userId='$userid'";

		mysql_query($qry_update_user) or dieLog(mysql_errno().": ".mysql_error()."<BR>");

		if($bank_update_sql && $adminInfo['li_level'] == 'full') mysql_query($bank_update_sql) or dieLog(mysql_errno().": ".mysql_error()."<BR>$bank_update_sql");
...
{
	document.getElementById('cd_paydelay').value=parseInt(document.getElementById('cd_paydaystartday').value)+parseInt(document.getElementById('cd_paydelayweeks').value)-parseInt(document.getElementById('cd_paystartday').value);
...
beginTable();
?>
<form onSubmit ="return func_ischanged()" action="editCompanyProfile3.php"  name="Frmcompany" id="Frmcompany" method="post">
...
<?php if(1){// $adminInfo['li_level'] == 'full') { ?>
                      ...
                      <tr>
                        ...
                        <td height="30" align="left" class='cl1'><?php if(1){// $adminInfo['li_level'] == 'full' || 1) { ?>
...
&nbsp;</td>
	...
	<? } ?>
					  <?php if(1){//($adminInfo['li_level'] == 'bank' && $adminInfo['li_type'] == 'credit') || $adminInfo['li_level'] == 'full') { ?>
					  <tr>
                        ...
                        <td height="30" align="left" class='cl1'><font size="1" face="Verdana, Arial, Helvetica, sans-serif">
                          <?=$cc_buyrate?>
                        </font></td>
                      </tr>
                      <tr>
                      ...
                      <tr>
                        ...
                        <td align="left" height="30" class='cl1'>&nbsp;
                          <select name="bank_Creditcard" style="font-family:arial;font-size:10px;width:120px">
                            ...
                            <?php
					func_fill_combo_conditionally("select bank_id, concat(bank_name,concat(' H-',concat(bk_fee_high_risk,concat('/L-',concat(bk_fee_low_risk,concat('/A-',bk_fee_approve)))))) as name from $str_bank_table and bk_cc_support=1 ORDER BY `bank_name` ASC ",$companyInfo['bank_Creditcard'],$cnn_cs);
				?>
                          </select>
                        </td>
                      </tr>
					...
					<script language="javascript">
					  ...
					  <?php } ?>
					  <?php if(1){//($adminInfo['li_level'] == 'bank' && $adminInfo['li_type'] == 'check') || $adminInfo['li_level'] == 'full') { ?>
					   <tr>
...
?>
                  ...
                  <td align="center" width="50%" valign="top"><table cellpadding='0' cellspacing='0' class='lefttopright' style='margin-top: 15; margin-bottom: 5' valign="top">
                      <tr>
						...
						?>
                      <tr>
                        <td height="30" colspan="2" align='left' class='cl1'><font face='verdana' size='1'><b>&nbsp;Company Completion</b></font></td>
                        <td height="30" class='cl1' align='left'><select name="cd_completion"><?php
						foreach($etel_completion_array as $key=>$data)
					echo "<option value='$key' style='font-weight:".$data['style']."' ".(intval($companyInfo['cd_completion'])==$key?"selected":"").">".$data['txt']."</option>";
						?></select>
                        </td>
                      </tr>
                      ...
                      <tr>
                        <td height="30" colspan="2" class='cl1'><strong><font size="1" face="Verdana, Arial, Helvetica, sans-serif">&nbsp;Allow Gateway Transactions </font></strong></td>
                        <td height="30" class='cl1'><input name="cd_allow_gatewayVT" type="checkbox" id="cd_allow_gatewayVT" value="1" <?=$companyInfo['cd_allow_gatewayVT'] == 1 ? "checked" : ""?>>
                        </td>
                      </tr>
                      <?php
				if ($companyInfo['transaction_type'] != "tele") {
...
<font face="verdana" size="1">after pay period ends </font></td>
                      ...
                      <tr>
                        <td height="31" colspan="2" align="left"  class='cl1' ><font face="verdana" size="1"> <strong>&nbsp; Next Pay Day </strong></font></td>
                        <td height="31" align="left"  class='cl1' >&nbsp;
                          <input name="cd_next_pay_day" type="text" id="cd_next_pay_day" style="font-family:arial;font-size:10px;width:100px" value="<?= $companyInfo['cd_next_pay_day'] ?>" maxlength="15">
                        </td>
                      </tr>
					  
					  <TR>
					  <td colspan="2" height="30" align="right" valign="center" class='cl1' bgcolor="#CCCCCC"><font face="verdana" size="1" color="#FFFFFF"><strong>Order Page Settings </strong>&nbsp;</font></td>
			<td colspan="2" height="30" class='cl1' align="left">&nbsp;</td>
		  </tr>
		  <tr>
			<td colspan="2" height="30" class='cl1'><strong><font size="1" face="Verdana, Arial, Helvetica, sans-serif">Order Page Method </font></strong></td>

			<td colspan="2" height="30" class='cl1'>
			  &nbsp;<select name="cd_orderpage_settings" id="cd_orderpage_settings" style="font-family:arial;font-size:10px;width:100px" onChange="displayverification();">
<?php	print func_get_enum_values('cs_companydetails','cd_orderpage_settings',$companyInfo['cd_orderpage_settings']); ?>
			  </select>
			  </td>
		  </tr>
		  <tr>
			<td colspan="2" height="30" class='cl1'><strong><font size="1" face="Verdana, Arial, Helvetica, sans-serif">Allow Customer Etelegate Accounts </font></strong></td>
			<td colspan="2" height="30" class='cl1'>
			  <input name="cd_orderpage_useraccount" type="checkbox" value="1" <?=$companyInfo['cd_orderpage_useraccount'] ? "checked" : ""?>></td>
		  </tr>
		  <tr>
			<td colspan="2" height="30" class='cl1'><strong><font size="1" face="Verdana, Arial, Helvetica, sans-serif">Approve Time Limit </font></strong></td>
			<td colspan="2" height="30" class='cl1'>
			  &nbsp;<input name="cd_approve_timelimit" type="text" style="font-family:arial;font-size:10px;width:100px"  value="<?=$companyInfo['cd_approve_timelimit']?>">
			  hours
			  </td>
		  </tr>
		  <tr>
			<td colspan="2" height="30" class='cl1'><strong><font size="1" face="Verdana, Arial, Helvetica, sans-serif">Fraud Score Limit (0 to 10)</font></strong></td>
			<td colspan="2" height="30" class='cl1'>
			  &nbsp;<input name="cd_fraudscore_limit" type="text" style="font-family:arial;font-size:10px;width:100px"  value="<?=$companyInfo['cd_fraudscore_limit']?>">
			  </td></TR>
...
<?php if( $adminInfo['li_level'] == 'full') { ?>
			...
			<?php
			endTable("Edit Company Rates and Fees","");
			?>
