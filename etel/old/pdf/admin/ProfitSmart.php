...
$my_sql['tables'] = array("cs_profit as pt");
...
$my_sql['return']["04|Source"] = array("source" => "if(td.reference_number is not null,
...
$my_sql['search']['pa.pa_status'] = array("input_type" => "select", "compare"=> "IN", "display"=>"Status");
$my_sql['search']['pa.pa_status']['options']['source']['pairs'] = "Status";
...
$my_sql['where']['1'] = array("value" => "1 and 2 in (pt_from_entity_ID,pt_to_entity_ID) ", "compare" => "=");
//and (td.td_non_unique=0 or status!='D')
	...
	$my_sql['subquery']['queries']['03|Revenue'] = array("name"=>"revenue", "source" => "CONCAT('\$',FORMAT(SUM(if(pt.pt_amount > 0,pt.pt_amount,0)),2),' (' ,SUM(if(pt.pt_amount > 0,1,0)) , ')')");
	$my_sql['subquery']['queries']['04|Deductions'] = array("name"=>"deductions", "source" => "CONCAT('\$',FORMAT(SUM(if(pt.pt_amount < 0,pt.pt_amount,0)),2),' (' ,SUM(if(pt.pt_amount < 0,1,0)) , ')')");
...
$my_sql['pairs']['Status'] = array(
	array("display" => "Pending", "value"=>"pending"),
	array("display" => "Fail", "value"=>"fail"),
	array("display" => "Success", "value"=>"success"),
	array("display" => "Void", "value"=>"void"),
	array("display" => "Payout Pending", "value"=>"payout_pending"),
	array("display" => "Payout Sent", "value"=>"payout_sent"),
	array("display" => "Payout Failed", "value"=>"payout_failed"),
	array("display" => "Payout Rollover", "value"=>"payout_rollover"),
	array("display" => "Delete", "value"=>"Delete")
	);	
...
if($subquery_group) 
{
	...
	{		
		case 'I':
			...
			if($export_data)
			{				
				$my_sql['subquery']['queries']['00|Invoice_En_Ref'] = array("name"=>"en_ref", "source" => "(select en_ref from cs_entities en1 where en1.en_ID = pa.pa_en_ID) ");
				$my_sql['subquery']['queries']['00|Invoice_UserID'] = array("name"=>"userID", "source" => "(select en_type_ID from cs_entities en1 where en1.en_ID = pa.pa_en_ID)");
			}
			break;	
		case 'T':
...
if($export_data)
{				
	$my_sql['return']["00|En_Ref"] = array("source" => "(select en_ref from cs_entities en1 where en1.en_ID = pa.pa_en_ID)","column"=>"en_ref");
}
/****************
