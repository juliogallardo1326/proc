...
//updatereportpage.php:	The admin page functions for displaying the company transactions. 
...
{  
	...
	$userid = (isset($HTTP_POST_VARS["userid"])?quote_smart($HTTP_POST_VARS["userid"]):"");
		...
		$email= (isset($HTTP_POST_VARS['email2'])?quote_smart($HTTP_POST_VARS['email2']):"");
	...
	if($crorcq1 =="C") {
		...
		if($amount)
		{
			...
			else
			{
					if($str_fields != "")
					{
						$qrt_update_details ="update cs_rebillingdetails set name='$firstname',surname='$lastname',phonenumber='$phonenumber',address='$address',checkorcard='C',CCnumber='".etelEnc($chequenumber)."',accounttype='$accounttype',country='$country',city='$city',state='$state',zipcode='$zip',checktype='$chequetype',amount=$amount,bankname='$bankname',bankroutingcode='$bankroutingcode',bankaccountnumber='$bankaccountno',misc='$misc',userid=$userid,voiceAuthorizationno='$voiceauth',shippingTrackingno='$shipping',socialSecurity='$socialno',driversLicense='$licenceno',billingDate='$setbilldate',licensestate='$licensestate' $str_fields where rebill_transactionid=$tid";
...
$strCurrentDateTime = func_get_current_date();
	  ...
	  $status= func_get_value_of_field($cnn_cs,"cs_transactiondetails","status","transactionId",$id);
	  
	  if(($cancelreason !="" || $other !="") &&($status=="A"))
	  {
	 		$return_insertId = func_transaction_updatenew($id,$cnn_cs);
	   
			$qrt_update_details ="Update cs_transactiondetails set cancelstatus='Y',admin_approval_for_cancellation = 'A' where transactionId=$return_insertId ";
			if(!($show_update_run = mysql_query($qrt_update_details,$cnn_cs)))
			{			dieLog(mysql_errno().": ".mysql_error()."<BR>");
			...
			else
			{
				 ...
				 {
/*					$msgtodisplay="This transaction has been already cancelled";
					$outhtml="y";
					message($msgtodisplay,$outhtml,$headerInclude);									
					exit();
*/				 } 
						...
						if($str_approval_status == "A") {
							if($crorcq1 == "C") {
								//if($strBankRoutingCode != "") {
									func_send_cancel_mail($userid,$crorcq1);
								//}
							}
						}
					}
					
			$qrt_update_details ="update cs_transactiondetails set reason='$cancelreason',cancellationDate='$canceldate',other='$other',admin_approval_for_cancellation = 'A' where transactionId=$return_insertId";
				if(!($show_update_sql =mysql_query($qrt_update_details,$cnn_cs)))
					{
						//function errorlog(date('Y-m-d H:i:s'),'admin-updatereportpage',$error_num,$error_desc,$cnn_cs)			dieLog(mysql_errno().": ".mysql_error()."<BR>");

					}
					else
					{
/*						$outhtml="y";
						$msgtodisplay="Selected transactions has been cancelled";
						message($msgtodisplay,$outhtml,$headerInclude);									
						exit();
*/					}
				 }
	...
	$routing_code = (isset($HTTP_POST_VARS['routing_code'])?quote_smart($HTTP_POST_VARS['routing_code']):"");	$radRange = (isset($HTTP_POST_VARS["radRange"])?quote_smart($HTTP_POST_VARS["radRange"]):"");
