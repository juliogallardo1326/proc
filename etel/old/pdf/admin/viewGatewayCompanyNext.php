...
//	$str_active_color = "red";
	...
	//$gatewayEmail = func_get_value_of_field($cnn_cs,"cs_companydetails","email","userId",$sessionGatewaylogin);

/*if($emailsingle !="")
	{
		$headers = "";
		$headers .= "From: Gateway <$gatewayEmail>\n"; 
		$headers .= "X-Sender: Gateway Admin\n"; 
		$headers .= "X-Mailer: PHP\n"; // mailer
		$headers .= "X-Priority: 1\n"; // Urgent message!
		$headers .= "Return-Path: <$gatewayEmail>\n"; // Return path for errors
	...
	if($companyname != "")
	{
		...
		if($companyname== "A" && $companytype== "A") 
		{
			if ($companytrans_type == "A") {
				$qry_select_companies = "select * from cs_companydetails where $str_gateway_qry ";
			} else {
				$qry_select_companies = "select * from cs_companydetails where $str_gateway_qry and transaction_type = '$companytrans_type' ";
			}
		}
		else if($companyname== "A"  && $companytype== "AC") 
		{
			if ($companytrans_type == "A") {
				$qry_select_companies = "select * from cs_companydetails where $str_gateway_qry and activeuser=1 ";
			} else {
				$qry_select_companies = "select * from cs_companydetails where $str_gateway_qry and transaction_type = '$companytrans_type' and activeuser=1 ";
			}
		}
		else if($companyname== "A" &&  $companytype== "NC") 
		{
			if ($companytrans_type == "A") {
				$qry_select_companies = "select * from cs_companydetails where $str_gateway_qry and activeuser=0 ";
			} else {
				$qry_select_companies = "select * from cs_companydetails where $str_gateway_qry and transaction_type = '$companytrans_type' and activeuser=0 ";
			}
		}
		else if($companyname== "A" &&  $companytype== "RE") 
		{
			if ($companytrans_type == "A") {
				$qry_select_companies = "select * from cs_companydetails where $str_gateway_qry and reseller_id <> '' ";
			} else {
				$qry_select_companies = "select * from cs_companydetails where $str_gateway_qry and transaction_type = '$companytrans_type' and reseller_id <> '' ";
			}
		}
		else if($companyname== "A" &&  $companytype== "ET") 
		{
			if ($companytrans_type == "A") {
				$qry_select_companies = "select * from cs_companydetails where $str_gateway_qry and reseller_id is null ";
			} else {
				$qry_select_companies = "select * from cs_companydetails where $str_gateway_qry and transaction_type = '$companytrans_type' and reseller_id is null ";
			}
		}
		 else 
		{
		...
		$sUploadedAllDocument 	=	(isset($HTTP_GET_VARS["chkUploadedDocuments"])?quote_smart($HTTP_GET_VARS["chkUploadedDocuments"]):"");
		$sCompletedApplication 	=	(isset($HTTP_GET_VARS["chkCompletedApplication"])?quote_smart($HTTP_GET_VARS["chkCompletedApplication"]):"");
		$sReadyToWire			=	(isset($HTTP_GET_VARS["chkReadyToWire"])?quote_smart($HTTP_GET_VARS["chkReadyToWire"]):"");
		$sExtra					=	"";
...
function downloadDocuments() {
	window.open("downloadGatewayDocumentsFromProfile.php?gatewayCompanies=<?=$gatewayCompanyId?>&companyname=<?=$companyname?>&chk_doc=yes&companytrans_type=<?=$companytrans_type?>&companymode=<?=$companytype?>&completed_uploading=<?=$sUploadedAllDocument?>&completed_application=<?=$sCompletedApplication?>&ready_to_wire=<?=$sReadyToWire?>",'',"'scrollbars=no,toolbar=no,menubar=no,resizable=no,width=500,height=200'");
}
</script>
...
<?
		...
		?>
				<p align="right">
				<a href="javascript:downloadDocuments()"><font face="verdana" size="1" style="margin-left:30"><b>Download Documents</b></font></a>&nbsp;&nbsp;
				</p>
		<?
			$i_active = 0;
			$i_non_active = 0;
			...
			?>
					<center>		
					<table cellspacing="0" cellpadding="0" style="margin-top:0" width="95%">
					<tr>
...
<?
			...
			if($showval[77] ==1 ) {
	?>
					<tr height='30'><td align='center' class='cl1'><font face='verdana' size='1' color="<?= $str_bg_color ?>">&nbsp;<a href="editCompanyProfile1.php?company_id=<?= $company_id?>" style="color:<?= $str_bg_color ?>"><strong><?=$showval[3]?></strong></a></font></td>
			<?php } else { ?>
					<tr height='30'><td align='center' class='cl1'><font face='verdana' size='1' color="<?= $str_bg_color ?>">&nbsp;<a href="editCompanyProfile1.php?company_id=<?= $company_id?>" style="color:<?= $str_bg_color ?>"><?=$showval[3]?></a></font></td>
			<?php } ?>
					...
					<?= $i_num_documents == 0 ? "0/6" : "<a href='viewGatewayUploads.php?companyname=$showval[0]&gatewayCompanies=$gatewayCompanyId&companymode=$companytype&companytrans_type=$companytrans_type&companies=$companyname' style='color:$str_bg_color'>$i_num_documents/6"?>
					</font></td> 
	...
	<!--	<tr><td colspan="2" align="right" valign="middle" height="20"><font face="verdana" size="1"><a href="#" onclick="showDocuments();">View Documents</a></font></td></tr> -->
		<tr>
