...
if($_POST['Action'] == "Remove Refunded Mark" && $adminInfo['li_level'] == 'full')
{
	...
	$sql = "update `cs_transactiondetails` set cancelstatus='N' where transactionId = '".$callNoteInfo['transaction_id']."'";
	$result = mysql_query($sql) or dieLog(mysql_error()." ~ $sql");
	$error_msg = "Refund Mark Removed (Note: No email sent out) ";
...
if( $adminInfo['li_level'] == 'full') $str_data .= "<td width='5%' bgcolor='#CCCCCC' ><span class='subhd'>Action</span></td>";		 
					  $str_data .= "</tr>";
			   }
					 ...
					 $str_data .= "<td bgcolor='#E2E2E2'><font size='1' face='Verdana' >";
						 ...
						 if($transInfo['cancelstatus'] == 'N') 
						 {
						 	$str_data .= "<input type='submit' name='Action' value='Refund' >";
						 	$str_data .= "<input type='submit' name='Action' value='Mark As Refunded' >";
						 }
						 else 
						 {
						 	$str_data .= "<strong>REFUNDED</strong><BR>";
						 	$str_data .= "<input type='submit' name='Action' value='Remove Refunded Mark' >";
						 }
