...
$siteid = $companyInfo['userId'];
$identity = " `cs_en_ID` = '".$companyInfo['en_ID']."'";
$userId = $companyInfo['userId'];

if($_GET['delete'])
{
	$qry_delete = "UPDATE `cs_company_sites` set cs_hide = '1' WHERE $identity AND `cs_ID` = '".$_GET['delete']."' LIMIT 1";
	sql_query_write($qry_delete) or dieLog(mysql_error(),"Failed to remove URL from this company");
...
if($_POST['mode'])
{
	...
	if ($_POST['mode'] == 'new') 
	{
		...
		if (mysql_num_rows($result)>0)
		{
			...
			sql_query_write($qry_update) or dieLog(mysql_error(). " ~ ".$qry_update,"Failed to reenable URL for this company. Please contact support.");
			toLog('pendingwebsite','merchant', "Merchant $userId reenables site $cs_URL", $deleted_cs_info['cs_ID']);
		}
		else
		{
			...
			sql_query_write($qry_update) or dieLog(mysql_error(). " ~ ".$qry_update,"Failed to add URL to this company. This URL may already exist for another company.");
			toLog('pendingwebsite','merchant', "Merchant $userId adds site $cs_URL",mysql_insert_id());
		}
	...
	else if ($_POST['mode'] == 'edit')
	{
		...
		$qry_update = "UPDATE `cs_company_sites` 
						SET 
							...
							`cs_verified` = if(`cs_verified` in ('declined','ignored'),'pending',`cs_verified`),
		...
		toLog('pendingwebsite','merchant', "Merchant $userId updates site $cs_URL", $cs_ID);
...
<a href="addwebsiteuser.php">Add a Website</a>
	...
	if($url['cs_verified']=='non-compliant') $url['cs_verified'] = 'approved';
