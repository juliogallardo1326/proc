<?php
/**
* This file has the landing page functions for search engines in it.
* It handles viewing all landing pages for a particular search engine (eg google)
* And it also handles viewing all landing pages for a particular search engine and keyword.
*
* @version     $Id: landingpages_engines.php,v 1.17 2005/10/20 03:32:14 chris Exp $
...
/**
* This class has the landing page functions for search engines in it.
* It handles viewing all landing pages for a particular search engine (eg google)
* And it also handles viewing all landing pages for a particular search engine and keyword.
*
* @package TrackPoint
* @subpackage TrackPoint_Functions
*/
class LandingPages_Engines extends TrackPoint_Functions {
	...
	*/
	function Process() {
		...
		if ($searchengine === false) {
			...
			$heading = $searchengine;
			$fullurl = $searchengine;
		}
		...
		if ($keywords) {
			$GLOBALS['TopHeading_Detail'] = stripslashes($keywords);
			$GLOBALS['TopHeading_Detail_Truncate'] = stripslashes($this->TruncateName($keywords));
		}

		$GLOBALS['Help'] = GetLang('HelpLandingPage_Engines');
		...
		$query = "SELECT COUNT(searchid) AS totalvisits, SUM(hasconversion) AS conv, SUM(amount) AS revenue, (SUM(hasconversion) / COUNT(searchid)*100) AS percent FROM " . TRACKPOINT_TABLEPREFIX . "search" . $total_subquery;
		...
		$query = "SELECT COUNT(searchid) AS totalvisits, SUM(hasconversion) AS conv, SUM(amount) AS revenue, (SUM(hasconversion) / COUNT(searchid)*100) AS percent FROM " . TRACKPOINT_TABLEPREFIX . "search WHERE " . $this->SearchUserID . ($this->CalendarRestrictions ? ' AND ' . $this->CalendarRestrictions : '');
		...
		$SearchTotalPercent = $this->FormatNumber(($row['percent'] * 100), 4);

		$template = $this->ParseTemplate('LandingPages', true, false);

		$query = "SELECT landingpage AS name, COUNT(searchid) AS visits, SUM(hasconversion) AS conv, SUM(amount) AS revenue, (SUM(hasconversion) / COUNT(searchid)*100) AS percent FROM " . TRACKPOINT_TABLEPREFIX . "search" . $total_subquery . " GROUP BY name";
		...
		while($row = $this->Db->Fetch($result)) {
			...
			$page = $row['name'];
			$alttitle = $page;
			$landingpagename = $this->TruncateName($page, 70);
