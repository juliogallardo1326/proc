	...
	<tr> 
		...
		<td align="left" valign="top">
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				...
				<tr> 
					<td background="{$tempdir}img/Image1.png" height="87"></td>
					<td></td>
				</tr>
			</table> 
		</td>
	...
	<tr> 
		...
		<td align="center" valign="top" background="{$tempdir}img/index_30.gif"> 
			<table width="760" border="0" cellspacing="0" cellpadding="0">
				<tr> 
					<td width="158" valign="top" height='100%'> 
						<table width="158" height='500px' border="0" cellspacing="0" cellpadding="0"  background="{$tempdir}img/Left1.gif">
{assign var="cnt" value="16"}
{foreach from=$main_header.links item=link}
{assign var="cnt" value="`$cnt+1`"}
							<tr><td height="25px" background="{$tempdir}img/index_{$cnt}.jpg" class="navigation"><a href="{$link.href}" class="a1"><font color="#000066">{$link.text}</font></a></td></tr>
{/foreach}              
							<tr> 
								<td height="100%">&nbsp;</td>
							</tr>
					...
					<td width="100%" align="center" valign="top" background="{$tempdir}img/index_17.gif">
						<table border="0" cellpadding="0" cellspacing="0" height="10" style="border-bottom-width:1; ">
		{if $sub_header.links}
							<tr>
		{foreach from=$sub_header.links item=link}<td height="15px" width="125px" class="{if $link.selected==1}rgtselected{else}blackrgt{/if}" valign="middle" align="center" background="{$tempdir}img/index_16b.jpg">{if $link.disabled!=1}<a href="{$link.href}" class="maintx">{$link.text}</a>{else}<span class="maindisabled" >{$link.text}</span>{/if}</td>{/foreach}
							</tr>
		{/if}
						</table>
		<!--submenu ends-->
		...
		{if $display_todo_list}
						...
						<table width="80%"  border="1" cellspacing="2" cellpadding="2">
							...
							<tr>
								<th scope="col" class="todoList" align="left">{$display_todo_list}</th>
							</tr>
						</table>
						</center>
            {else}
			  {if $display_stat_wait} 
						<div id="hidewait" align="center"><br><img src="{$tempdir}images/stats_wait.gif" width="355" height="33">
						</div>
			{/if}
		{/if}