...
$my_sql['search']['td.userId'] = array("input_type" => "hidden", "compare"=> "=","required"=>1,"value" => $sessionlogin);
...
if($export_data)
{
	unset($my_sql['return']);
	$my_sql['return'] = array(
		array("source" => "td.reference_number as ReferenceID"),
		array("source" => "td.userid as CompanyID"),
		array("source" => "cd.companyname as CompanyName"),
		array("source" => "DATE_FORMAT(td.transactionDate,'%Y-%m-%d %H:%i:%s') as TransDate"),
		array("source" => "td.productdescription as ProductDescription"),
		array("source" => "concat(td.surname,', ',td.name) AS FullName"),
		array("source" => "td.amount AS Amount"),
		array("source" => "if(td.status!='D', if(td.status='P','Pending', 'Approved'), 'Declined'	) as Status"),
		array("source" => "if(td.td_is_chargeback=1,'Yes','No') as Chargeback"),
		array("source" => "if(td.cancelstatus='Y','Yes','No') as Refund"),
		array("source" => "ss.ss_subscription_ID as SubscriptionId"),
		array("source" => "ss.ss_rebill_status as SubscriptionStatus"),
		);
	if(in_array($export_data,array('full','transaction')))
		$my_sql['limit']['forcelimit']=30000;
	$_REQUEST['frm_timelimit'][0] = 1;
}
...
if($subquery_group) 
{
	...
	$my_sql['suborderby'] = array("transactionDate asc");
	$sql_date_range = "' (From ', DATE_FORMAT(min(transactionDate),'%b %D'),' To ', DATE_FORMAT(max(transactionDate),'%b %D'),')'";
	switch($subquery_group)
	{
		case 'D':
			$my_sql['subquery']['queries']['00|Declines'] = array("name"=>"subgroup_by", "source" => "CONCAT('(',count(*),') ',td_process_msg )");
			$my_sql['subgroupby'] = array("td_process_msg");
			$my_sql['suborderby'] = array("number_transactions desc");
			$detail=1;
			break;
		case 1:
			$my_sql['subquery']['queries']['00|Date'] = array("name"=>"subgroup_by", "source" => "DATE_FORMAT( transactionDate , '%W, %M %D %Y' )");
			break;
		case 7:
			$my_sql['subquery']['queries']['00|Date'] = array("name"=>"subgroup_by", "source" => "CONCAT( DATE_FORMAT( transactionDate , '%M, Week #%U' ), $sql_date_range)");
			$my_sql['subgroupby'] = array("DATE_FORMAT( transactionDate , '%U' )");
			break;
		case 30:
			$my_sql['subquery']['queries']['00|Date'] = array("name"=>"subgroup_by", "source" => "CONCAT( DATE_FORMAT( transactionDate , '%M' ), $sql_date_range)");
			$my_sql['subgroupby'] = array("DATE_FORMAT( transactionDate , '%M' )");
			break;
		case 60:
		case 90:
		case 180:
			$my_sql['subquery']['queries']['00|Date'] = array("name"=>"subgroup_by", "source" => "CONCAT( '".intval($subquery_group/30)." Months ',$sql_date_range )");
			$my_sql['subgroupby'] = array("ROUND(MONTH( transactionDate )/".intval($subquery_group/30).")");
			break;
		case 360:
			$my_sql['subquery']['queries']['00|Date'] = array("name"=>"subgroup_by", "source" => "CONCAT(DATE_FORMAT( transactionDate , '%Y' ), $sql_date_range )");
			$my_sql['subgroupby'] = array("DATE_FORMAT( transactionDate , '%Y' )");
			break;
		default:
			$my_sql['subquery']['queries']['00|Date'] = array("name"=>"subgroup_by", "source" => "CONCAT($sql_date_range)");
			$my_sql['subgroupby'] = array("FLOOR(TO_DAYS(transactionDate)/($subquery_group))");
			break;
	}
}
...
if(smart_process_mysql_form($my_sql))
{
	...
	{	
		...
		if($export_data=='transaction') $my_sql['skip_subquery']=true;
	}
	...
	{	
		...
		flush();
		if(in_array($export_data,array('full','summary')))
		{
			$line = '';
			
				foreach($result['sub_row'][0] as $key=>$row)
				{
					$line .= ($line?",":"").'"'.ucwords(str_replace(array('_','"'),array(' ','`'),$key)).'"';
				}
				echo $line."\n";
				flush();
		...
		{
			$line = '';
			foreach($result['rows'] as $keyrow)
				foreach($keyrow[0] as $key=>$data)
				{
					$line .= ($line?",":"").'"'.str_replace('"','`',$key).'"';
				}
				echo $line."\n";
				flush();
				foreach($keyrow as $row)
				{
					$line = '';
					foreach($row as $key=>$data)
					{
