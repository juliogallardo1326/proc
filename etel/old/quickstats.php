...
if(!$showgraph)
{
	...
	beginTable();
		...
		echo "<table width='100%' class='invoice'>";
		echo "
		<tr>
		...
		echo "<tr class='infoHeader'><td></td>";
		...
		foreach($timeArray as $t=>$d)
		{
			...
			echo "<td class='infoHeader'><a href='".$d['link']."'>$t</a></td>";
	...
	echo "<a href='javascript:showGraph(\"?showgraph=1&mb=2&style=full&cs_ID=$cs_ID\")'><img src='?showgraph=1&mb=2&style=tiny&cs_ID=$cs_ID' alt='No Data Available' border='0' ></a>";
	echo "<a href='javascript:showGraph(\"?showgraph=1&mb=1&style=full&cs_ID=$cs_ID\")'><img src='?showgraph=1&mb=1&style=tiny&cs_ID=$cs_ID' alt='No Data Available' border='0' ></a><br>";
	echo "<a href='javascript:showGraph(\"?showgraph=1&mb=4&style=full&cs_ID=$cs_ID\")'><img src='?showgraph=1&mb=4&style=tiny&cs_ID=$cs_ID' alt='No Data Available' border='0' ></a>";
	echo "<a href='javascript:showGraph(\"?showgraph=1&mb=3&style=full&cs_ID=$cs_ID\")'><img src='?showgraph=1&mb=3&style=tiny&cs_ID=$cs_ID' alt='No Data Available' border='0' ></a><br>";
	echo "</div>";
...
else
{
	...
	$thismonth = strtotime(" -$monthsback month",$thismonth);
	$daysinmonth = intval(date('t',$thismonth));
	...
	$monthstart = strtotime(" -$monthsback month",$thismonth);
	$monthend = strtotime(" +1 month",$monthstart);
	$my_sql['where']['td.transactionDate'] = array("value" => "'".date('Y-m-01 00:00:00',$monthstart)."' and '".date('Y-m-d 00:00:00',$monthend)."'", "compare" => "between");
	...
	$Graph->add(
		Image_Graph::vertical(
			Image_Graph::factory('title', array('Daily Sales for '.date('F',$thismonth), 12)),
