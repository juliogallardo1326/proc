<?php
require_once('int.refund.php');
/*
function execute_refund($transInfo,$reason)
{
	global $cnn_cs;
	...
	$sql="SELECT * FROM `cs_companydetails` as c left join {$database["database_main"]}.`cs_company_sites` as s on s.cs_company_id = c.`userId` WHERE `cs_gatewayId` = ".$_SESSION["gw_id"]." AND c.`userId` = '".$transInfo['userId']."' and s.`cs_ID` = '".$transInfo['td_site_ID']."'";
...
function IPG_refund_integration($transInfo,$bankInfo,$companyInfo)
{
	...
	$DESC = "0001etelegate.net 1800-6760127";
	...
	"orderDescription" => strval($DESC),
	"refundamount" => strval("USD"),
	"referralOrderReference" => strval("0"),
	"comment1" => "" . (number_format($transInfo['amount'],2)) . ""
	);
	print_r($params);die();
	...
	$response=NULL;
	list($ResponseNum,$ResponseMsg) = split(' - ',$process_result['RESPONSECODE']);
	
	$response['errormsg'] = $process_result['ERRORCODE'] . ": $ResponseNum " . $process_result['ERRORMESSAGE'];
	...
	$response['td_bank_transaction_id'] = $process_result['ORDERREFERENCE'];
	$response['td_process_result'] = serialize($process_result);
	$response['td_process_query'] = serialize($processor->query);
