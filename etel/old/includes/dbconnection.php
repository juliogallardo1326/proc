<?php 
	...
	$sql = "SELECT * FROM {$database['database_main']}.`etel_gateways`";
	...
	if(!$etel_gw_list[$gateway_db_select]) unset($gateway_db_select);
	// select Gateway
   	if(!$gateway_db_select ) 
	{
		if($_SESSION['gw_id'] == '1' || ($_SESSION['gw_user_username'] == 'demo' && $_SESSION['gw_user_password'] == 'demo'))
			...
			$gateway_db_select = array_pop(array_keys($etel_gw_list));
	}
		if (!$gateway_db_select) die("Invalid Access $gateway_db_select");
	$gwInfo=$etel_gw_list[$gateway_db_select];
	if($gwInfo) foreach($gwInfo as $k=>$d) $_SESSION[$k]=$d;
	...
	{
		if(ip2long($etel_current_ip)!=$gwInfo['gw_debug_ip']) sql_query_write("update etel_dbsmain.etel_gateways set gw_debug_ip = '". ip2long($etel_current_ip)."' where gw_id = '".$gwInfo['gw_id']."'");
		$etel_debug_mode=1; 
	}
