...
function payneteasy_integration(&$transInfo,&$bankInfo,&$companyInfo)
{
	set_time_limit(120);
	$type = 'VISA';
	...
	$cust_cntry = func_get_country($transInfo['country'],'co_ISO');
	...
	$Pinfo="version=1";	// 	  10  A-N 	  1  The version number of the API you are using. 	
	$Pinfo.="&sid=".urlencode($bankInfo['bk_username']);	//	  12  Numeric 	  567654165  Your client account number. 	
	$Pinfo.="&client_orderid=1";//.urlencode($transInfo['reference_number']);	//	  12  Numeric 	  12345  The unique identifying number for this order. 	
	$Pinfo.="&order_desc=".urlencode($transInfo['productdescription']);	//	  125  A-N 	  digital product  A brief description of your field of enterprise. 	
	$Pinfo.="&first_name=".urlencode($transInfo['name']);	//	  50  A-N 	  sam  The customer’s first name. 	
	$Pinfo.="&last_name=".urlencode($transInfo['surname']);	//	  50  A-N 	  ng  The customer’s last name. 	
	$Pinfo.="&ssn=".rand(1000,9999);	//	  4  Numeric 	  <left empty>  The last four digits of the customer’s social security number. 	
	//$Pinfo.="&birthday=".urlencode($transInfo['']);	//	  6  Numeric 	  032081  The customer’s birthday in the format MMDDYY. 	
	$Pinfo.="&address1=".urlencode($transInfo['address']);	//	  50  A-N 	  huanghou road 162  The customer’s address line 1. 	
	$Pinfo.="&address2=".urlencode($transInfo['address2']);	//	  50  A-N 	  <left empty>  The customer’s address line 2. 	
	$Pinfo.="&city=".urlencode($transInfo['city']);	//	  50  A-N 	  HongKong  The customer’s city. 	
	$Pinfo.="&state=".urlencode($transInfo['state']);	//	  2  A-N 	  <left empty>  The customer’s US states (two letter abbreviation). For a list of valid state codes, please see Appendix A. Not applicable outside the US. 	
	$Pinfo.="&zip_code=".urlencode($transInfo['zipcode']);	//	  10  A-N 	  10024  The customer’s ZIP code. 	
	$Pinfo.="&country=".urlencode($cust_cntry);	//	  2  A-N 	  CN  The customer’s country(two letter abbreviation). For a list of valid country codes, please see Appendix B. 	
	$Pinfo.="&phone=".urlencode($transInfo['phonenumber']);	//	  15  A-N 	  (852) 123 4567  The customer’s full international phone number, including country suffix. 	
	//$Pinfo.="&cell_phone=".urlencode($transInfo['']);	//	  15  A-N 	  123456789  The customer’s full international cell phone number, including country suffix. 	
	$Pinfo.="&email=".urlencode($transInfo['email']);	//	  50  A-N 	  sam@yahoo.com  The customer’s email address. 	
	$Pinfo.="&amount=".urlencode($transInfo['amount']);	//	  10  Numeric 	  100  The amount charged. 	
	$Pinfo.="&currency=USD";	//	  3  A-N 	  USD  The currency the transaction is charged in (three letter abbreviation). The valid parameter values are:  USD for US Dollar  EUR for European Euro 	
	$Pinfo.="&credit_card_number=".urlencode($transInfo['CCnumber']);	//	  20  Numeric 	  4444333322221111  Customer credit number. 	
	$Pinfo.="&expire_month=".urlencode($expMonth);	//	  2  Numeric 	  11  The credit card’s month of expiration. 	
	$Pinfo.="&expire_year=".urlencode($expYear);	//	  2  Numeric 	  09  The credit card’s year of expiration. 	
	$Pinfo.="&cvv2=".urlencode($transInfo['cvv']);	//	  3 – 4  Numeric 	  625  The customer’s CVV2 code. CVV2 (Card Verification Value) is the three of four digit number farthest to the right on the flip side of a credit card. 	
	$Pinfo.="&ipaddress=".urlencode($transInfo['ipaddress']);	//	  20  A-N 	  218.39.216.234  The customer’s IP address, include for fraud screening purposes. 	
	//$Pinfo.="&redirect_success_url=http://www.etelegate.com&redirect_failure_url=http://www.etelegate.com&server_callback_url=http://www.etelegate.com";
	$query_url = "https://www.payneteasy.com/paynet/payment.html?";
	...
	$result_str = http_post2('ssl://www.payneteasy.com', 443, $output_url, $Pinfo);
	toLog('order','customer', "PayNetEasy Result for ".$transInfo['reference_number'].":  ".$process_result,$transInfo['transactionId']);

	parse_str($result_str,$process_result);
	$response=NULL;
	$response['errormsg'] = $process_result['error_code'] . ": " . $process_result['error_message'];
	$response['td_bank_transaction_id'] = $process_result['orderid'];
	$response['td_process_result'] = $result_str;
...
function intabill_integration(&$transInfo,&$bankInfo,&$companyInfo)
{
	...
	$processor = new intabill_Client();
	$res = $processor->Execute_Sale(&$transInfo,&$bankInfo,&$companyInfo);
	return $res;
	/*
...
function ch_CheckGateway_integration($transInfo,$bankInfo,$companyInfo)
{
	...
	if(intval($transInfo['phonenumber'])<999999999) $transInfo['phonenumber'] = '6022654322';
	if($transInfo['phonenumber']=='1231231234') $transInfo['phonenumber'] = '6022654322';
	...
	// Uncomment below for live
	$output_url = "https://www.checkgateway.com/EPNPublic/ACH.aspx";
