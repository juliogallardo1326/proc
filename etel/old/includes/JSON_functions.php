...
function JSON_get_data($qry)
{
	...
	switch($qry['func'])
	{
		...
		case 'getEntityList':	
			...
			if(!$sql_info['sort']) $sql_info['sort'] = " ORDER BY en_ID DESC ";
			
			if($sql_info['where']) $sql_info['where'] = " AND ( ".$sql_info['where']. " )";
			if(!$curUserInfo['en_access'][ACCESS_AUTH_ENTITY_ADMIN]) $sql_info['where'] .= " AND en_type != 'admin' ";
			...
			if($sql_info['join']) $sql_info['group'] = " Group by en_ID ";
			$info['TimeOut']=5;
