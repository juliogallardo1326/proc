...
function smart_updateMarkup($values,$action,$updateall=false)
{
	...
	$sql = "select ce.en_ID as merchant_en_ID, merchant_contract_agree,cb_ID,cb_config
			FROM 
	...
	if($userid || !$updateall)
		$sql .= " and cd.userId = '$userid'";
		
	if($updateall)
		$sql .= " and merchant_contract_agree = 1";
				
	$markupResult = sql_query_read($sql) or dieLog(mysql_error()." ~ $sql");
	if(!mysql_num_rows($markupResult))
		return array("action"=>"Merchant(s) Not Found.","status"=>"fail");
