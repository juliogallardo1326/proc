...
class User
{
    ...
    function updateSMS($usr_id, $sms_email)
    {
        $stmt = "UPDATE
                 ...
                 SET
                    en_ev_sms_email='" . Misc::escapeString($sms_email) . "'
                 WHERE
                    en_ID=" . Misc::escapeInteger($usr_id);
     ...
     */
    function getActiveAssocList($prj_id = false, $role = NULL, $exclude_grouped = false, $grp_id = false, $adminonly = false)
    {
		...
		// START ETEL MODIFIED
        if ($adminonly) {
            $stmt .= " AND en_type = 'admin' ";
        }
		// END ETEL MODIFIED
        if ($grp_id != false) {
            if ($exclude_grouped == false) {
                $stmt .= " AND (en_ev_grp_id IS NULL OR en_ev_grp_id = $grp_id)";
            } else {
                $stmt .= " AND en_ev_grp_id = $grp_id";
            }
        } elseif ($exclude_grouped == true) {
            $stmt .= " AND en_ev_grp_id IS NULL";
        }
        $stmt .= "
                 ORDER BY
                    en_firstname ASC";
	...
	// START ETEL MODIFIED
        $stmt = "SELECT
                 ...
                 FROM
                    " . ETEL_USER_TABLE_NOSUB . "
                 WHERE
                    en_ID IN (" . implode(', ', Misc::escapeInteger($items)) . ")";
		...
		if (PEAR::isError($res)) {
			...
			if(sizeof($res))
				...
				{
					$name = trim($r[0]);
					if( Auth::getCurrentRole() >= User::getRoleID("Manager") && $name)
					{
						switch($r[1])
						{
							case 'merchant':
								$name = "<a target='_top' href='/admin/editCompanyProfileAccess.php?entity_id=".$r[2]."'>$name</a>";
								break;
    ...
    function changeStatus()
    {
        ...
        $stmt = "UPDATE
                    " . ETEL_USER_TABLE_NOSUB . "
                 SET
                    en_status='" . $HTTP_POST_VARS["status"] . "'
                 WHERE
                    en_ID IN ($items)";
        $res = $GLOBALS["db_api"]->dbh->query($stmt);
        if (PEAR::isError($res)) {
    ...
    function updatePassword($usr_id, $send_notification = FALSE)
    {
		...
		$username = $GLOBALS["db_api"]->dbh->getOne("SELECT en_username FROM
                    " . ETEL_USER_TABLE_NOSUB . " WHERE en_ID='$usr_id'");
		
        $stmt = "UPDATE
                    " . ETEL_USER_TABLE_NOSUB . "
                 SET
                    en_password='" . Auth::hashPassword($username.$HTTP_POST_VARS["new_password"]) . "'
                 WHERE
                    en_ID=" . Misc::escapeInteger($usr_id);
        $res = $GLOBALS["db_api"]->dbh->query($stmt);
        if (PEAR::isError($res)) {
    ...
    function update()
    {
        ...
        $stmt = "UPDATE
                 ...
                 SET
                    ...
                    en_lastname='" . Misc::escapeString($fn[1]) . "'
                    en_email='" . Misc::escapeString($HTTP_POST_VARS["email"]) . "'";
        if (!empty($HTTP_POST_VARS["password"]) && 0) {
		
		
			$username = $GLOBALS["db_api"]->dbh->getOne("SELECT en_username FROM
						" . ETEL_USER_TABLE_NOSUB . " WHERE en_ID='".Misc::escapeInteger($HTTP_POST_VARS["id"])."'");
		
            $stmt .= ",
                    usr_password='" . Auth::hashPassword($username.$HTTP_POST_VARS["password"]) . "'";
        }
        $stmt .= "
                 WHERE
                    en_ID=" . Misc::escapeInteger($HTTP_POST_VARS["id"]);
