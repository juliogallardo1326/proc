<?php
/**
* $Id: category.php,v 1.20.2.16.2.6 2006/05/03 11:22:28 thorstenr Exp $
*
	...
	function viewTree()
    {
        ...
        for ($y = 0 ;$y < $this->height(); $y = $this->getNextLineTree($y)) {
            ...
            if (isset($PMF_CONF['mod_rewrite']) && $PMF_CONF['mod_rewrite'] == "TRUE") {
                $output .= sprintf('<a title="%s" href="category%d.html">%s</a>%s', $description, $parent, $categoryName, $num_entries);
            } else {
                $output .= sprintf('<a title="%s" href="?%saction=show&amp;cat=%d">%s</a>%s', $description, $sids,  $parent, $categoryName, $num_entries);
            }
