...
else
{
	...
	if (sizeof($daily['sub_row'])>15)
		$AxisX->setFontSize(8);     
	else $AxisX->setFontSize(10);     
	
	if($style!='full')
	{
		if (sizeof($daily['sub_row'])>15)
			$AxisX->setLabelInterval(5);   
		else if (sizeof($daily['sub_row'])>10)
			$AxisX->setLabelInterval(2);
	}
	...
	foreach($daily['sub_row'] as $index => $data)
	{
		...
		if($data['amount_approved']) $Datasets[0]->addPoint(intval($index), floatval($data['amount_approved']));
		if($style=='full')
		{
			if($data['amount_rebilled']) $Datasets[1]->addPoint(intval($index), floatval($data['amount_rebilled']));
			if($data['amount_newsales']) $Datasets[2]->addPoint(intval($index), floatval($data['amount_newsales']));
		}
