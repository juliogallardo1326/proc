...
function process_rebills(&$trans_class,$cntLimit=0,$settings=NULL)
{
	...
	echo "</table>";
	echo "Total Amount Processed: $totalamount<BR> $cnt < $cntLimit || $cntLimit==0) && '".print_r($rebill)."'";
...
while($dayinfo_fetch = mysql_fetch_assoc($result))
	$dayinfo[$dayinfo_fetch['ss_bank_id']] = $dayinfo_fetch;
	
$bankList = array(
	18 => array('cnt_limit'=>5000,'amt_limit'=>28000),
	...
	//41 => array('cnt_limit'=>7000,'amt_limit'=>70000,'bill_cnt'=>120,'bank_where'=>"ss_billing_cvv2 is not null"),
	//42 => array('cnt_limit'=>7000,'amt_limit'=>70000,'bill_cnt'=>120,'bank_where'=>"ss_billing_cvv2 is not null"),
	//46 => array('cnt_limit'=>1200,'amt_limit'=>12000,'bill_cnt'=>120,'bank_where'=>"ss_billing_cvv2 is not null"),
	//47 => array('cnt_limit'=>1200,'amt_limit'=>12000,'bill_cnt'=>120,'bank_where'=>"ss_billing_cvv2 is not null"),
...
foreach($bankList as $bankid => $bankItem)
{
	...
	{	$bankItem['skip'] = true;	$bankItem['skipped_because'] = "Bill Count < 1".print_r($bankItem,true); }
	
	if($bankItem['cnt_limit'] && $bankItem['cnt']>$bankItem['cnt_limit'])
	{	$bankItem['skip'] = true;	$bankItem['skipped_because'] = "Count Limit ".$bankItem['cnt'].">".$bankItem['cnt_limit']; }
	if($bankItem['amt_limit'] && $bankItem['amt']>$bankItem['amt_limit'])
	{	$bankItem['skip'] = true;	$bankItem['skipped_because'] = "Amount Limit ".$bankItem['amt'].">".$bankItem['amt_limit']; }
