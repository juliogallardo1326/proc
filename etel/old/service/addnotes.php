<?php
	...
	$i_call_note_id = (isset($HTTP_POST_VARS["hid_call_note_id"])?quote_smart($HTTP_POST_VARS["hid_call_note_id"]):"");
	$strTransactionId = (isset($HTTP_POST_VARS["hdTransactionId"])?quote_smart($HTTP_POST_VARS["hdTransactionId"]):"");
	$strNotes = (isset($HTTP_POST_VARS["txtNotes"])?quote_smart($HTTP_POST_VARS["txtNotes"]):"");
	$str_chk_cancel = (isset($HTTP_POST_VARS["chkCancel"])?quote_smart($HTTP_POST_VARS["chkCancel"]):"");
	$str_callback = (isset($HTTP_POST_VARS["chkCallBack"])?quote_smart($HTTP_POST_VARS["chkCallBack"]):"");
	$i_bill_year = (isset($HTTP_POST_VARS["opt_bill_year"])?quote_smart($HTTP_POST_VARS["opt_bill_year"]):"");
	$i_bill_month = (isset($HTTP_POST_VARS["opt_bill_month"])?quote_smart($HTTP_POST_VARS["opt_bill_month"]):"");
	$i_bill_day = (isset($HTTP_POST_VARS["opt_bill_day"])?quote_smart($HTTP_POST_VARS["opt_bill_day"]):"");
	$str_approval_status = (isset($HTTP_POST_VARS["hid_approval_status"])?quote_smart($HTTP_POST_VARS["hid_approval_status"]):"");
	$str_bill_date = (isset($HTTP_POST_VARS["hid_bill_date"])?quote_smart($HTTP_POST_VARS["hid_bill_date"]):"");
	$str_orig_bill_date = (isset($HTTP_POST_VARS["hid_orig_bill_date"])?quote_smart($HTTP_POST_VARS["hid_orig_bill_date"]):"");
	$str_call_duration = (isset($HTTP_POST_VARS["hid_counter"])?quote_smart($HTTP_POST_VARS["hid_counter"]):"");
	$str_solved = (isset($HTTP_POST_VARS["chkSolved"])?quote_smart($HTTP_POST_VARS["chkSolved"]):"0");
	$str_dnc = (isset($HTTP_POST_VARS["chkDNC"])?quote_smart($HTTP_POST_VARS["chkDNC"]):"N");
	$str_changed_bill_date = "N"; 
	//$str_changed_bill_date = (isset($HTTP_POST_VARS["chkChangedBillDate"])?quote_smart($HTTP_POST_VARS["chkChangedBillDate"]):"N");

	// hidden values for building the body of the DNC Mail.

	$str_first_name = (isset($HTTP_POST_VARS["hid_first_name"])?quote_smart($HTTP_POST_VARS["hid_first_name"]):"");
	$str_last_name = (isset($HTTP_POST_VARS["hid_last_name"])?quote_smart($HTTP_POST_VARS["hid_last_name"]):"");
	$str_company_name = (isset($HTTP_POST_VARS["hid_company_name"])?quote_smart($HTTP_POST_VARS["hid_company_name"]):"");
	$str_telephone_number = (isset($HTTP_POST_VARS["hid_telephone_number"])?quote_smart($HTTP_POST_VARS["hid_telephone_number"]):"");
	$str_address = (isset($HTTP_POST_VARS["hid_address"])?quote_smart($HTTP_POST_VARS["hid_address"]):"");
	$str_city = (isset($HTTP_POST_VARS["hid_city"])?quote_smart($HTTP_POST_VARS["hid_city"]):"");
	$str_state = (isset($HTTP_POST_VARS["hid_state"])?quote_smart($HTTP_POST_VARS["hid_state"]):"");
	$str_zip = (isset($HTTP_POST_VARS["hid_zip"])?quote_smart($HTTP_POST_VARS["hid_zip"]):"");
	$str_country = (isset($HTTP_POST_VARS["hid_country"])?quote_smart($HTTP_POST_VARS["hid_country"]):"");
	$i_user_id = (isset($HTTP_POST_VARS["hid_user_id"])?quote_smart($HTTP_POST_VARS["hid_user_id"]):"");
	$str_check_or_card = (isset($HTTP_POST_VARS["hid_check_or_card"])?quote_smart($HTTP_POST_VARS["hid_check_or_card"]):"");
	...
	if ($str_chk_cancel=="Y")
	{
		...
		if($strCurrentDate1 >= $str_new_bill_date && $str_approval_status == "Approved")
		{
			/*if($str_set_query == "")
				...
				$str_set_query .= ",reason='Customer Service',cancellationDate='$strDateTime',admin_approval_for_cancellation='P'";*/
			$qrt_update_details ="Update cs_transactiondetails set reason='Customer Service',cancellationDate='$strDateTime' where transactionId=$strTransactionId and userId = $i_user_id";
	...
	$qryUpdate = "update cs_callnotes set service_notes='Customer Service Refund',customer_notes='$strNotes', cancel_status='$str_cancel_status',is_bill_date_changed='$str_changed_bill_date',solved='$str_solved',call_duration='$str_call_duration',customer_service_id=$i_customer_service_id,prev_bill_date='$str_orig_bill_date',dnc='$str_dnc' where note_id = $i_call_note_id";
	if(!mysql_query($qryUpdate)) {
